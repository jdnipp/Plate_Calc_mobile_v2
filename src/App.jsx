...
const SETUPS = {
  acuostandard: { name: "ACUO Standard", unit: "lb", barWeight: 45, collarWeight: 0, plates: [45, 25, 15, 10, 5, 2.5], inventory: { 45: 12, 25: 6, 15: 4, 10: 6, 5: 6, 2.5: 4 }, favorites: [95, 135, 155, 185, 205, 225, 275, 315], target: 185 },
  competitionkg: { name: "Competition KG", unit: "kg", barWeight: 20, collarWeight: 0, plates: [25, 20, 15, 10, 5, 2.5, 1.25], inventory: { 25: 10, 20: 2, 15: 2, 10: 4, 5: 4, 2.5: 4, 1.25: 4 }, favorites: [60, 80, 100, 120, 140], target: 100 },
};

const STORAGE_KEYS = { favorites: "acuo-plate-calculator-favorites", setup: "acuo-plate-calculator-setup", inventoryMode: "acuo-plate-calculator-inventory", unitMode: "acuo-plate-calculator-unit" };

const roundToHundredth = (v) => Math.round(v * 100) / 100;
const formatNumber = (v) => (v === null || v === undefined || Number.isNaN(v)) ? "" : (Number.isInteger(v) ? String(v) : String(v).replace(/\.0$/, ""));

function groupPlates(plates) {
  const counts = {};
  plates.forEach((p) => counts[p] = (counts[p] || 0) + 1);
  return Object.entries(counts).sort((a, b) => Number(b[0]) - Number(a[0])).map(([plate, count]) => ({ plate: Number(plate), count }));
}

function formatPlateList(plates, unitLabel) {
  if (!plates.length) return "No plates needed";
  return groupPlates(plates).map(({ plate, count }) => `${count} × ${formatNumber(plate)} ${unitLabel}`).join(", ");
}

function plateHeight(plate) {
  const map = {55:78,45:74,35:68,25:62,20:58,15:54,10:48,5:40,2.5:34,1.25:28};
  return map[plate] || 32;
}
...
function getClosestSolution(targetWeight, barWeight, collarWeight, availablePlates, inventoryMode, inventory) {
  ...
  function dfs(index, currentWeight, currentPlates, remainingInventory) {
    ...
    if (diff < best.diff || (diff === best.diff && currentWeight > best.weight)) {
      best = { plates: [...currentPlates], weight: roundToHundredth(currentWeight), diff: roundToHundredth(diff) };
    }
    ...
    const maxByInventory = inventoryMode ? Math.floor((remainingInventory[plate] || 0) / 2) : Math.max(0, Math.floor((perSideTarget + best.diff) / plate));
    ...
    for (let count = maxCount; count >= 0; count--) {
      ...
      if (inventoryMode && count > 0) nextInventory[plate] = (nextInventory[plate] || 0) - count * 2;
...
function calculatePlates({ targetWeight, barWeight, collarWeight, availablePlates, inventoryMode, inventory }) {
  if (targetWeight < barWeight + collarWeight * 2) return { error: "Target weight must be at least the bar plus collars.", info: "", perSide: [], perSideWeight: 0, totalLoaded: roundToHundredth(barWeight + collarWeight * 2), exact: false };
  const perSideTarget = roundToHundredth((targetWeight - barWeight - collarWeight * 2) / 2);
  ...
  for (const plate of availablePlates) {
    ...
    if (inventoryMode) maxCount = Math.min(maxCount, Math.floor((remainingInventory[plate] || 0) / 2));
  ...
  if (exact) return { error: "", info: collarWeight > 0 ? `Includes ${formatNumber(collarWeight)} per collar.` : "", perSide, perSideWeight, totalLoaded, exact: true };
...
function BarbellDiagram({ perSide, unitLabel, barWeight, collarWeight }) {
  ...
  const right = [...perSide];
  const dense = (perSide.length * 2 + (collarWeight > 0 ? 2 : 0)) > 8;
      ...
      <div className={`barbell-fit ${dense ? "dense" : ""}`}>
        <div className="plate-side">
          {left.map((plate, index) => {
            ...
            return <div key={`left-${plate}-${index}`} className="plate realistic" style={{ height: `${plateHeight(plate)}px`, background: style.bg, borderColor: style.border, color: style.text }} title={`${formatNumber(plate)} ${unitLabel}`}><span>{formatNumber(plate)}</span></div>;
          })}
          {collarWeight > 0 ? <div className="collar">C</div> : null}
        </div>
        <div className="bar-section" />
        <div className="bar-center short-bar"><strong>{formatNumber(barWeight)} {unitLabel}</strong><span>bar</span></div>
        <div className="bar-section" />
        <div className="plate-side">
          {collarWeight > 0 ? <div className="collar">C</div> : null}
          {right.map((plate, index) => {
            const style = plateStyle(plate, unitLabel);
            return <div key={`right-${plate}-${index}`} className="plate realistic" style={{ height: `${plateHeight(plate)}px`, background: style.bg, borderColor: style.border, color: style.text }} title={`${formatNumber(plate)} ${unitLabel}`}><span>{formatNumber(plate)}</span></div>;
          })}
        </div>
      </div>
    </div>
...
function SetupButton({ active, children, onClick }) {
  return <button className={active ? "pill active" : "pill"} onClick={onClick} type="button">{children}</button>;
}
...
export default function App() {
  ...
  const [unitLabel, setUnitLabel] = useState(() => {
    if (typeof window === "undefined") return activeSetup.unit;
    return localStorage.getItem(STORAGE_KEYS.unitMode) || activeSetup.unit;
  });
  ...
  const [inventoryMode, setInventoryMode] = useState(() => typeof window === "undefined" ? false : localStorage.getItem(STORAGE_KEYS.inventoryMode) === "true");
  ...
  useEffect(() => { localStorage.setItem(STORAGE_KEYS.setup, setupMode); }, [setupMode]);
  useEffect(() => { localStorage.setItem(STORAGE_KEYS.inventoryMode, String(inventoryMode)); }, [inventoryMode]);
  useEffect(() => { localStorage.setItem(STORAGE_KEYS.unitMode, unitLabel); }, [unitLabel]);

  useEffect(() => {
    const nextMode = unitLabel === "kg" ? "competitionkg" : "acuostandard";
    const next = SETUPS[nextMode];
  ...
  const availablePlates = useMemo(() => plateInput.split(",").map((p) => Number(p.trim())).filter((p) => !Number.isNaN(p) && p > 0).sort((a, b) => b - a), [plateInput]);
  ...
  function applySetup(nextMode) { const next = SETUPS[nextMode]; setSetupMode(nextMode); setUnitLabel(next.unit); }
  function updateInventoryCount(plate, value) { const parsed = Math.max(0, Number(value) || 0); setInventory((prev) => ({ ...prev, [plate]: parsed })); }
  function addFavorite() { const value = Number(favoriteDraft); if (!Number.isNaN(value) && value > 0 && !favorites.includes(value)) { setFavorites((prev) => [...prev, value].sort((a, b) => a - b)); setFavoriteDraft(""); } }
  function removeFavorite(weight) { setFavorites((prev) => prev.filter((item) => item !== weight)); }
  ...
  async function handleShare() {
    ...
    try {
      if (navigator.share) { await navigator.share({ title: "ACUO Plate Calculator", text }); setShareMessage("Shared."); }
      else if (navigator.clipboard) { await navigator.clipboard.writeText(text); setShareMessage("Copied setup to clipboard."); }
      else { setShareMessage("Sharing is not supported on this device."); }
    } catch { setShareMessage("Share was canceled."); }
  }
    ...
    <div className="app-shell">
      <div className="phone-frame">
        ...
        <div className="top-controls no-print">
          <div className="unit-toggle" role="tablist" aria-label="Unit toggle">
            <button className={unitLabel === "lb" ? "unit-btn active" : "unit-btn"} type="button" onClick={() => setUnitLabel("lb")}>LB</button>
            <button className={unitLabel === "kg" ? "unit-btn active" : "unit-btn"} type="button" onClick={() => setUnitLabel("kg")}>KG</button>
          </div>
          <div className="setup-row">
            {Object.entries(SETUPS).map(([key, setup]) => <SetupButton key={key} active={setupMode === key} onClick={() => applySetup(key)}>{setup.name}</SetupButton>)}
          </div>
        </div>

        <div className="card result-card sticky-result">
          {!result ? <p className="subtle">Enter valid numbers to calculate plates.</p> : <>
            <div className="hero-result"><span>Per side</span><strong>{formatPlateList(result.perSide, unitLabel)}</strong></div>
        ...
        <div className="card">
          ...
          <div className="grid one">
            <label className="field"><span>Target</span><input inputMode="decimal" type="number" value={targetWeight} onChange={(e) => setTargetWeight(e.target.value)} /></label>
            ...
            <div className="quick-picks no-print">
              {favorites.map((weight) => <button key={weight} type="button" className="pill large" onClick={() => setTargetWeight(String(weight))}>{weight}</button>)}
            </div>
            <div className="two-up">
              <label className="field"><span>Bar</span><input inputMode="decimal" type="number" value={barWeight} onChange={(e) => setBarWeight(e.target.value)} /></label>
              <label className="field"><span>Collar</span><input inputMode="decimal" type="number" value={collarWeight} onChange={(e) => setCollarWeight(e.target.value)} /></label>
            </div>
          </div>
        </div>

        {result ? <>
          <div className={result.exact ? "notice success" : "notice warning"}><strong>{result.exact ? "Exact match" : result.error}</strong>{result.info ? <p>{result.info}</p> : null}</div>
        ...
        <details className="advanced no-print">
          ...
          <div className="card small-card">
            <label className="field"><span>Available Plate Sizes</span><input value={plateInput} onChange={(e) => setPlateInput(e.target.value)} /><small>Comma separated plate sizes in {unitLabel}.</small></label>
            <div className="toggle-row">
              <div><strong>Gym inventory mode</strong><p className="subtle">Optional. Leave off for normal class use.</p></div>
            ...
            {inventoryMode ? <div className="grid two inventory-grid">
              {availablePlates.map((plate) => <label className="field compact" key={plate}><span>{formatNumber(plate)} {unitLabel}</span><input type="number" min="0" value={inventory[plate] ?? 0} onChange={(e) => updateInventoryCount(plate, e.target.value)} /></label>)}
            </div> : null}
            <div className="favorites-editor">
              <h3>Quick lifts</h3>
              <div className="favorites">
                {favorites.map((weight) => <div key={weight} className="favorite-chip"><button type="button" onClick={() => setTargetWeight(String(weight))}>{weight} {unitLabel}</button><button type="button" className="remove" onClick={() => removeFavorite(weight)}>×</button></div>)}
              </div>
              <div className="add-favorite"><input type="number" value={favoriteDraft} onChange={(e) => setFavoriteDraft(e.target.value)} placeholder={`Add quick lift ${unitLabel}`} /><button type="button" className="primary" onClick={addFavorite}>Add</button></div>
            </div>
          </div>
        </details>
