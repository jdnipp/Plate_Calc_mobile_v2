...
const parsePlates = (s) =>
  s
    ...
    .sort((a, b) => b - a);
const plateHeight = (p) =>
  ({ 55: 82, 45: 78, 35: 72, 25: 66, 20: 62, 15: 58, 10: 52, 5: 44, 2.5: 38, 1.25: 32 }[p] || 36);
...
function closest(target, bar, collar, plates, inventoryMode, inventory) {
  ...
  function dfs(i, w, cur, inv) {
    ...
    if (diff < best.diff || (diff === best.diff && w > best.weight))
      best = { plates: [...cur], weight: round2(w), diff: round2(diff) };
...
function calculate({
  ...
  if (remaining === 0)
    return {
      error: "",
      info: collarWeight > 0 ? `Includes ${fmt(collarWeight)} per collar.` : "",
      perSide,
      perSideWeight,
      totalLoaded,
      exact: true,
...
export default function App() {
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE.setup, setupMode);
  }, [setupMode]);
  useEffect(() => {
    localStorage.setItem(STORAGE.inventory, String(inventoryMode));
  }, [inventoryMode]);
  ...
  const result = useMemo(() => {
    const target = Number(targetWeight),
      bar = Number(barWeight),
      collar = Number(collarWeight);
    if (
      Number.isNaN(target) ||
      Number.isNaN(bar) ||
      Number.isNaN(collar) ||
  ...
  function addFavorite() {
    const v = Number(favoriteDraft);
    if (!Number.isNaN(v) && v > 0 && !favorites.includes(v)) {
      setFavorites((prev) => [...prev, v].sort((a, b) => a - b));
      setFavoriteDraft("");
    }
  }
  function removeFavorite(weight) {
    setFavorites((prev) => prev.filter((x) => x !== weight));
  }
            ...
            <div className="bar-options">
              {activeSetup.barOptions.map((w) => (
                <button
                  key={w}
                  type="button"
                  className={Number(barWeight) === w ? "pill active" : "pill"}
                  onClick={() => setBarWeight(String(w))}
                >
                  {w} lb bar
                </button>
              ))}
